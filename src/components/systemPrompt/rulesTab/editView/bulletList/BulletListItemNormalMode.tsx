...
// ** Hooks
import useRulesInformation, { PromptComponentsEnum } from 'customHooks/bullets';
import useRulesBulletFunctions from 'customHooks/bullets';
...
const BulletListItemNormalMode = ({
  ...
  // Contexts
  const { tempRulesData } = useRulesContext();
  const { deleteBullet, moveUpBullet, moveDownBullet } = useBulletFunctions(PromptComponentsEnum.RULE);

  // States
  const [isExpanded, setIsExpanded] = useState(false);

  const optionInitialValue: string = tempRulesData[index].option;
  const textInitialValue: string = tempRulesData[index].text;
      ...
      <div className="relative bg-white flex flex-col pt-2 pb-0 px-2 rounded">
        <div className="flex h-[40px] justify-between items-center">
          ...
          <div className="flex items-center justify-center gap-2 ml-2">
            <button
              disabled={index === tempRulesData.length - 1}
              onClick={() => {
                setIsExpanded(false);
                moveDownBullet(index);
              }}
              className={`${
                index === tempRulesData.length - 1 ? 'bg-gray-400' : 'bg-gray-200'
