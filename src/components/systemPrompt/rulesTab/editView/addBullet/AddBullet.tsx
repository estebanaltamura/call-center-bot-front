...
const AddBulletSection = ({ isEditing }: { isEditing: boolean }) => {
  ...
  // Contexts
  const { addBullet } = useBulletFunctions(PromptComponentsEnum.RULE);
  ...
  // Agregar bullet
  const addRulesInformationItemHandler = () => {
  ...
  useEffect(() => {
    const updatedUsed = tempRulesData.map((item) => item.option);
    ...
    <div className="border border-gray-400 p-4 bg-gray-50 rounded space-y-4">
      ...
      <div className="flex flex-col items-center space-y-2">
        ...
        <button
          ...
          onClick={addRulesInformationItemHandler}
