...
// ** Types
import { Entities, IKnowledgeContextEntity, StateTypes } from 'types/dynamicSevicesTypes';
...
const MainViewItem = ({ docItem }: { docItem: IKnowledgeContextEntity }) => {
  const { handleModifyDoc, currentKnowledgeContext } = useKnowledgeContextContext();

  const isActive = currentKnowledgeContext?.title === docItem.title;

  const deleteKnowledgeContextHandler = () => {
    if (isActive) {
    ...
    if (confirm('Confirma que quieres eliminar este documento')) {
      SERVICES.CMS.softDelete(Entities.knowledgeContext, docItem.id);
    } else {
      return;
    }
  };

  const reactivateKnowledgeContextHandler = () => {
    if (confirm('Confirma que quieres reactivar este documento')) {
      SERVICES.CMS.reactivateSoftDeleted(Entities.knowledgeContext, docItem.id);
    } else {
      return;
    }
  };
    ...
    >
      ...
      <div className="flex space-x-2">
        ...
        {docItem.state === StateTypes.active ? (
          <button
            onClick={deleteKnowledgeContextHandler}
          ...
          <button
            onClick={reactivateKnowledgeContextHandler}
