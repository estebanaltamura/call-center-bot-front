...
import useRulesInformation from 'customHooks/rules/rulesInformation';
...
const RulesInformationListItem = ({
  infoItem,
  index,
  tempRulesInformationLength,
}: {
  infoItem: IOptionTextItem;
  index: number;
  tempRulesInformationLength: number;
}) => {
  const { setTempRulesInformation } = useRulesContext();

  const { deleteRulesInformationItem, moveUpRulesInformationItem, moveDownRulesInformationItem } =
    useRulesInformation();
      ...
      <div className="relative bg-white flex h-[45px] justify-first items-center">
        ...
        <div className="flex items-center justify-center gap-2 ml-2">
          {isEditing ? (
            ...
            <>
              <button
                disabled={index === tempRulesInformationLength - 1}
                onClick={() => {
                  setIsExpanded(false);
                  moveDownRulesInformationItem(index);
                }}
                className={`${
                  index === tempRulesInformationLength - 1 ? 'bg-gray-400' : 'bg-gray-200'
              ...
              <button
                ...
                onClick={() => {
                  setIsExpanded(false);
                  moveUpRulesInformationItem(index);
                }}
              ...
              <button
                onClick={() => deleteRulesInformationItem(index)}
...
export default RulesInformationListItem;
